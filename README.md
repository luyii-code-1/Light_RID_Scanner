...
The service starts on `http://<device-ip>:4600/`. On first run with no `config.json`, the web UI enters the OOBE setup flow.
...
The portable edition automatically disables authentication, notifications, and host monitoring.
...
The `--config` flag can point to any path:
