...
function showSettingsPageLoading(target, title){
  ...
  settingsLoadingTimer = setInterval(tick, 1000);
}
...
function renderSettingsTasks(){
  ...
  host.innerHTML = settingsTasks.map(function(task){
    ...
    return '<div class="settings-task ' + stateClass + '">'
      + '<div class="settings-task-icon" aria-hidden="true"></div>'
      + '<div class="settings-task-copy"><div class="settings-task-title">' + enc(String(task.title || '正在处理')) + '</div>'
      + '<div class="settings-task-detail">' + enc(String(task.detail || '')) + '</div></div></div>';
  }).join('');
}
...
async function withSettingsAsyncTask(title, detail, fn){
  ...
  try{
    var result = await fn({
      update: function(nextDetail){ updateSettingsTask(taskId, {detail: String(nextDetail || detail || '')}); }
    });
...
function connectSettingsRuntimeWs(){
  ...
  if(settingsRuntimeWs){
    try{ settingsRuntimeWs.close(); }catch(_e){}
  ...
  settingsRuntimeWs.onclose = function(){
    ...
    settingsRuntimeWsReconnectTimer = setTimeout(connectSettingsRuntimeWs, 2000);
  };
}
...
function bindShellActions(){
  ...
  on('btn-diagnostic-export', 'click', async function(){
    ...
    try{
      if(btn) btn.disabled = true;
      await withSettingsAsyncTask('诊断导出', '正在生成质量分析包...', function(){ return downloadQualityReport(); });
    }catch(e){
  ...
  on('btn-refresh-runtime', 'click', function(){
    withSettingsAsyncTask('运行数据', '正在读取运行状态...', function(){ return guarded(loadRuntimePanel, 'status-runtime', '运行数据已刷新。', 1800, 3600); });
  });
  on('btn-reload-view', 'click', function(){
    withSettingsAsyncTask('设置重载', '正在重新读取设置...', function(){ return guarded(loadVisual, 'status-visual', '设置已重新读取。', 2200); });
  });
...
function bindDataTransferActions(){
  on('btn-export-settings-file', 'click', function(){
    withSettingsAsyncTask('设置文件导出', '正在导出设置文件...', function(){ return guarded(exportSettingsFile, 'status-data-transfer', '设置文件已导出。', 2200, 3600); });
  });
  ...
  on('import-settings-file', 'change', function(ev){
    ...
    if(!file) return;
    withSettingsAsyncTask('设置文件导入', '正在导入设置文件...', function(){ return guarded(function(){ return importSettingsFileFromFile(file); }, 'status-data-transfer', '', 0, 4200); });
  });
  on('btn-export-scan-data', 'click', function(){
    withSettingsAsyncTask('扫描数据导出', '正在导出扫描数据...', function(){ return guarded(exportScanDataFile, 'status-data-transfer', '扫描数据已导出。', 2200, 3600); });
  });
  on('btn-import-scan-data', 'click', function(){ pickFileInput('import-scan-data-file'); });
  on('import-scan-data-file', 'change', function(ev){
    var file = (ev && ev.target && ev.target.files && ev.target.files[0]) ? ev.target.files[0] : null;
    if(!file) return;
    withSettingsAsyncTask('扫描数据导入', '正在导入扫描数据...', function(){ return guarded(function(){ return importScanDataFileFromFile(file); }, 'status-data-transfer', '', 0, 4200); });
  });
  on('btn-reidentify-recent-history', 'click', function(){
    withSettingsAsyncTask('历史包重解析', '正在识别所有飞机近100包...', function(){ return guarded(reidentifyRecentHistoryPackets, 'status-data-transfer', '所有飞机近100个存储包已识别。', 2400, 5200); });
  });
}
...
function bindCaptureActions(){
  on('btn-network-bind-refresh', 'click', function(){ withSettingsAsyncTask('网卡扫描', '正在扫描网卡...', function(){ return guarded(refreshNetworkBindings, 'status-network-bind', '网卡列表已刷新。', 1800, 3600); }); });
  on('btn-network-bind-save', 'click', saveNetworkBindingsToDraft);
  on('btn-network-bind-apply', 'click', function(){ withSettingsAsyncTask('网卡绑定应用', '正在应用网卡绑定...', function(){ return guarded(applyNetworkBindings, 'status-network-bind', '网卡绑定已应用。', 2600, 5200); }); });
  on('network-bind-module', 'toggle', function(){
    var host = qs('network-bind-module');
    if(host && host.open){
      withSettingsAsyncTask('网卡扫描', '正在扫描网卡...', function(){ return refreshNetworkBindings(); }).catch(function(e){ setStatus('status-network-bind', e.message || e, true); });
    }
  });
...
function bindSystemServiceActions(){
  on('btn-service-refresh', 'click', function(){
    withSettingsAsyncTask('服务状态', '正在读取服务状态...', function(){ return guarded(loadSystemServiceStatus, 'status-system-service', '服务状态已刷新。', 1800, 3600); });
  });
...
function bindRawActions(){
  on('btn-load-raw', 'click', function(){
    withSettingsAsyncTask('原始配置读取', '正在读取原始配置...', function(){ return guarded(loadRaw, 'status-raw', '原始配置已读取。', 2200); });
  });
  on('btn-save-raw', 'click', function(){
    withSettingsAsyncTask('原始配置保存', '正在保存原始配置...', function(){ return guarded(saveRaw, 'status-raw', '原始配置已保存。', 2600); });
  });
  on('btn-delete-raw', 'click', function(){
    withSettingsAsyncTask('原始配置删除', '正在删除原始配置文件...', function(){ return guarded(deleteRawFile, 'status-raw', '原始配置文件已删除。', 2600); });
  });
  ...
  on('raw-tree', 'click', function(ev){
    ...
    rawRefreshButtons();
    withSettingsAsyncTask('原始配置切换', '正在切换配置文件...', function(){ return guarded(function(){ return rawLoadFile(path); }, 'status-raw', '已切换配置文件。', 1800); });
  });
}
function bindSaveActions(){
  on('btn-test-visual', 'click', async function(){
    try{
      setVisualActionBusy(true);
      await withSettingsAsyncTask('配置测试', '正在测试当前配置...', function(){ return testVisual(); });
    }catch(e){
      setStatus('status-visual', e.message || e, true);
      showNotice(e.message || e, 'warn', 3800);
    }finally{
  ...
  on('btn-save-visual', 'click', async function(){
    try{
      setVisualActionBusy(true);
      await withSettingsAsyncTask('配置保存', '正在保存设置...', function(){ return saveVisual(); });
    }catch(e){
      setStatus('status-visual', e.message || e, true);
      showNotice(e.message || e, 'warn', 3800);
    }finally{
...
function initializeSettingsPage(){
  ...
  withSettingsPageLoading('Station 设置', '正在读取设置', async function(){
    await loadVisual();
  }).catch(function(e){ setStatus('status-visual', e.message || e, true); showNotice(e.message || e, 'warn', 3800); });
  window.setTimeout(function(){
