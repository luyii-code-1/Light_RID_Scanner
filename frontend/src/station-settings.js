...
function bindShellActions(){
  ...
  on('btn-refresh-host', 'click', function(){
    withSettingsPageLoading('Station 设置', '正在读取设置', function(){ return guarded(loadVisual, 'status-visual'); });
  });
  on('btn-refresh-runtime', 'click', function(){
    withSettingsPageLoading('运行数据', '正在读取运行状态', function(){ return guarded(loadRuntimePanel, 'status-runtime', '运行数据已刷新。', 1800, 3600); });
  });
  on('btn-reload-view', 'click', function(){
    withSettingsPageLoading('Station 设置', '正在读取设置', function(){ return guarded(loadVisual, 'status-visual', '设置已重新读取。', 2200); });
  });
...
function bindDataTransferActions(){
  on('btn-export-settings-file', 'click', function(){
    withSettingsPageLoading('设置文件', '正在导出设置文件', function(){ return guarded(exportSettingsFile, 'status-data-transfer', '设置文件已导出。', 2200, 3600); });
  });
  ...
  on('import-settings-file', 'change', function(ev){
    ...
    if(!file) return;
    withSettingsPageLoading('设置文件', '正在导入设置文件', function(){ return guarded(function(){ return importSettingsFileFromFile(file); }, 'status-data-transfer', '', 0, 4200); });
  });
  on('btn-export-scan-data', 'click', function(){
    withSettingsPageLoading('扫描数据', '正在导出扫描数据', function(){ return guarded(exportScanDataFile, 'status-data-transfer', '扫描数据已导出。', 2200, 3600); });
  });
  on('btn-import-scan-data', 'click', function(){ pickFileInput('import-scan-data-file'); });
  on('import-scan-data-file', 'change', function(ev){
    var file = (ev && ev.target && ev.target.files && ev.target.files[0]) ? ev.target.files[0] : null;
    if(!file) return;
    withSettingsPageLoading('扫描数据', '正在导入扫描数据', function(){ return guarded(function(){ return importScanDataFileFromFile(file); }, 'status-data-transfer', '', 0, 4200); });
  });
  on('btn-reidentify-recent-history', 'click', function(){
    withSettingsPageLoading('历史包识别', '正在识别所有飞机近100包', function(){ return guarded(reidentifyRecentHistoryPackets, 'status-data-transfer', '所有飞机近100个存储包已识别。', 2400, 5200); });
  });
}
...
function bindCaptureActions(){
  on('btn-network-bind-refresh', 'click', function(){ withSettingsPageLoading('网卡绑定', '正在扫描网卡', function(){ return guarded(refreshNetworkBindings, 'status-network-bind', '网卡列表已刷新。', 1800, 3600); }); });
  on('btn-network-bind-save', 'click', saveNetworkBindingsToDraft);
  on('btn-network-bind-apply', 'click', function(){ withSettingsPageLoading('网卡绑定', '正在应用网卡绑定', function(){ return guarded(applyNetworkBindings, 'status-network-bind', '网卡绑定已应用。', 2600, 5200); }); });
  on('network-bind-module', 'toggle', function(){
    var host = qs('network-bind-module');
    if(host && host.open){
      withSettingsPageLoading('网卡绑定', '正在扫描网卡', refreshNetworkBindings).catch(function(e){ setStatus('status-network-bind', e.message || e, true); });
    }
  });
...
function bindSystemServiceActions(){
  on('btn-service-refresh', 'click', function(){
    withSettingsPageLoading('systemd 服务', '正在读取服务状态', function(){ return guarded(loadSystemServiceStatus, 'status-system-service', '服务状态已刷新。', 1800, 3600); });
  });
...
function bindRawActions(){
  on('btn-load-raw', 'click', function(){
    withSettingsPageLoading('原始配置', '正在读取原始配置', function(){ return guarded(loadRaw, 'status-raw', '原始配置已读取。', 2200); });
  });
  on('btn-save-raw', 'click', function(){
    withSettingsPageLoading('原始配置', '正在保存原始配置', function(){ return guarded(saveRaw, 'status-raw', '原始配置已保存。', 2600); });
  });
  on('btn-delete-raw', 'click', function(){
    withSettingsPageLoading('原始配置', '正在删除原始配置文件', function(){ return guarded(deleteRawFile, 'status-raw', '原始配置文件已删除。', 2600); });
  });
  ...
  on('raw-tree', 'click', function(ev){
    ...
    rawRefreshButtons();
    withSettingsPageLoading('原始配置', '正在切换配置文件', function(){ return guarded(function(){ return rawLoadFile(path); }, 'status-raw', '已切换配置文件。', 1800); });
  });
}
...
function initializeSettingsPage(){
  ...
  withSettingsPageLoading('Station 设置', '正在读取设置', async function(){
    await loadVisual();
    await refreshNetworkBindings().catch(function(e){ setStatus('status-network-bind', e.message || e, true); });
